...
func tableTechnologies(_ context.Context) *plugin.Table {
    return &plugin.Table{
        Name:        "detectify_technologies",
...
func listTechnologies(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
    ...
    if err != nil {
        plugin.Logger(ctx).Error("detectify_technologies.listTechnologies", "connection_error", err)
        return nil, err
    }
    ...
    for _, splitResponse := range paginatedResponse {
        ...
        if err != nil {
            plugin.Logger(ctx).Error("detectify_technologies.listTechnologies", "failed_unmarshal", err)
            return nil, err
        }

        for i, finding := range response.TechItems {
