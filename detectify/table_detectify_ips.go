...
func tableIps(_ context.Context) *plugin.Table {
    return &plugin.Table{
        Name:        "detectify_ips",
...
func listIps(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
    ...
    if err != nil {
        plugin.Logger(ctx).Error("detectify_ips.listIps", "connection_error", err)
        return nil, err
    }
    ...
    for _, splitResponse := range paginatedResponse {
        ...
        if err != nil {
            plugin.Logger(ctx).Error("detectify_ips.listIps", "failed_unmarshal", err)
            return nil, err
        }

        for i, finding := range response.IPItems {
